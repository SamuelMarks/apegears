...
class TypeIntegrationTest(unittest.TestCase):
    ...
    def test_fields(self):
        ...
        self.assertEqual(
            P("x", type=TypeMinimal, default="zz", cli_args="").x.val, "zz"
        )
    ...
    def test_enum(self):
        ...
        self.assertEqual(
            P("x", type=Enum1, default=Enum1.coo, cli_args="").x, Enum1.coo
        )
        self.assertEqual(P("x", type=Enum1, cli_args="-x foo").x, Enum1.foo)
        self.assertEqual(
            P("x", type=Enum1, default=Enum1.coo, cli_args="-x bar").x, Enum1.bar
        )
        ...
        self.assertEqual(
            P("x", type=Enum1, choices=choices, cli_args="-x bar").x, Enum1.bar
        )
        self.assertRaises(
            SystemExit, P, "x", type=Enum1, choices=choices, cli_args="-x coo"
        )
    ...
    def test_enum_list(self):
        ...
        self.assertRaises(
            SystemExit, P, "x", type=Enum1, choices=two_vals, cli_args="-x coo"
        )
        self.assertRaises(SystemExit, P, "x", type=Enum1, cli_args="-x no-such-value")
    ...
    def test_enum_dict(self):
        ...
        self.assertEqual(P("x", type=Enum1, cli_args="").x, {})
        self.assertEqual(
            P("x", type=Enum1, cli_args="-x k1=foo k2=bar").x,
            {"k1": Enum1.foo, "k2": Enum1.bar},
        )
        self.assertEqual(
            P("x", type=Enum1, cli_args="-x k1=foo -x k2=bar").x,
            {"k1": Enum1.foo, "k2": Enum1.bar},
        )
        self.assertRaises(
            SystemExit, P, "x", type=Enum1, cli_args="-x k1=no-such-value"
        )
    ...
    def test_date(self):
        ...
        self.assertEqual(
            self._parse("positional", type="date", cli_args="%s" % d).date, d
        )
        self.assertEqual(
            self._parse("list", type="date", cli_args="--date %s" % d).date, [d]
        )
    ...
    def test_datetime(self):
        ...
        self.assertEqual(
            self._parse("positional", type="datetime", cli_args="%s" % tstr).timestamp,
            t,
        )
        self.assertEqual(
            self._parse(
                "list", type="datetime", cli_args="--timestamp %s" % tstr
            ).timestamp,
            [t],
        )
