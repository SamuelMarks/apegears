...
class AddArgumentTest(unittest.TestCase):
    ...
    def test_flag(self):
        ...
        self.assertRaises(
            SystemExit, P, "xx", include_negative=False, cli_args="--no-xx"
        )
    ...
    def test_list(self):
        ...
        self.assertEqual(
            P("x", type=int, cli_args="-x -2 4 -x 6 8 10").x, [-2, 4, 6, 8, 10]
        )
    ...
    def test_dict(self):
        ...
        self.assertEqual(P("x", cli_args="-x aa=bb").x, {"aa": "bb"})
        self.assertEqual(
            P("x", type=int, cli_args="-x aa=1 bb=2 -x cc=3").x,
            {"aa": 1, "bb": 2, "cc": 3},
        )
        self.assertEqual(
            P("x", key_type=int, type=float, cli_args="-x 1=1.5 2=2.5 -x 3=3.5").x,
            {1: 1.5, 2: 2.5, 3: 3.5},
        )
